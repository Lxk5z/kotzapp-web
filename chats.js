...
// ===================================
// ⬆️⬇️ TASTATUR-NAVIGATION DURCH CHATS
// ===================================
...
function updateSelection() {
  ...
  // Markiere Items als "keyboard-selected"
  if (selectedIndex >= 0) {
    items.forEach(i => i.classList.add("has-selection"));
  } else {
    items.forEach(i => i.classList.remove("has-selection"));
  }
...
document.addEventListener("keydown", (e) => {
  ...
  // Wenn noch nichts ausgewählt ist → Hover als Startpunkt
  if (selectedIndex === -1 && hoverIndex !== -1) {
...
document.addEventListener("mousemove", (e) => {
  // Nur echte Mausbewegung erkennen
  if (e.clientX === lastMouseX && e.clientY === lastMouseY) return;

  lastMouseX = e.clientX;
  lastMouseY = e.clientY;

  // Keyboard-Auswahl entfernen
  document.querySelectorAll(".chat-item.has-selection")
    .forEach(i => i.classList.remove("has-selection"));

  document.querySelectorAll(".chat-item.selected")
...
function clearChatKeyboardSelection() {
  document.querySelectorAll(".chat-item.selected")
    .forEach(i => i.classList.remove("selected"));
